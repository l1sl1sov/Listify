...
import { getHighlighted } from '@/utils/task'
...
const Task = ({ task, searchValue, selectParams }: TaskProps) => {
  ...
  return (
    <li className={styles.task} key={task.task_id}>
      <div className="flex flex-row gap-2 items-center">
        ...
        >
          ...
          <div className={styles.texts}>
            ...
            <div className='inline-block'>
              {searchValue && task.description ? (
                getHighlighted(searchValue, task.description).map(
                  (part, index) =>
                    part.ifHighlighted ? (
                      <mark key={index} className='bg-blue-500'>{part.text}</mark>
                    ) : (
      ...
      <Modal isOpen={isUpdatingModal} onClose={() => setIsUpdatingModal(false)}>
